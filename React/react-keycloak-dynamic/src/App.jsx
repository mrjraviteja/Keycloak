...
import keycloakLogo from './assets/keycloak.png'
  ...
  kc.init({
    ...
    else
    {
      ...
      if (kc.tokenParsed.preferred_username === 'appuser') {
        console.log('Valid user. Proceeding to React page...');
      } 
      else 
      {
        console.log('Restricted user. Redirecting to Keycloak login page with message...');
        kc.logout({redirectUri: 'http://localhost:5173/?message=invalid_user'})
      }
  ...
  return (
    <>
      ...
      <div className='grid'>
        ...
        <button onClick={() => {setInfoMessage(kc.tokenParsed.preferred_username)}} className='btn'>Show user name</button>
      </div>
